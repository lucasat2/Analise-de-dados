...
--------------------------------------------------------------------------------------------------------------------------------
2- Qual a média de tempo (em segundos) de duração do aluguel de bike 
por tipo de membro e por estação fim 
(onde as bikes são entregues após o aluguel)?
...
SELECT tipo_membro,estacao_fim,
...
SELECT CAST(data_inicio as date) AS data_inicio, estacao_inicio
...
6- Retornar: Estação fim, data fim de cada aluguel de bike e duração de cada aluguel em segundos; Número de aluguéis de bikes (independente da estação) 
ao longo do tempo; Somente os registros quando a data fim foi no mês de Abril.
...
# Estação fim, 
data fim 
e duração em segundos do aluguel 
...
considerando data de início do aluguel 
e estação de início?
